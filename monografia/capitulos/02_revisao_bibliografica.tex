...
Em conjuntos fortemente desbalanceados, a acurácia pode permanecer alta mesmo quando o classificador falha na classe positiva. Curvas e áreas de precisão--revocação são particularmente informativas nesse cenário \cite{saito2015precision}. Por isso, este trabalho emprega AUC-PR, AUC-ROC, precisão, revocação, F1 e matriz de confusão, acompanhadas do limiar usado. O limiar não deve ser escolhido no conjunto de teste.

A regressão logística foi usada como linha de base por oferecer um ponto de comparação simples e reproduzível. Random Forest representa uma alternativa não linear baseada na agregação de árvores \cite{breiman2001random}, enquanto XGBoost usa \emph{boosting} de árvores com mecanismos de regularização e escalabilidade \cite{chen2016xgboost}. Comparar esses modelos permite verificar se ganhos de complexidade se traduzem em melhoria relevante para a classe rara.
...
Mudança de conceito limita a interpretação de uma divisão aleatória como evidência operacional. Um modelo pode reconhecer padrões presentes simultaneamente em treino e teste sem manter desempenho em período posterior. Assim, uma separação temporal é metodologicamente preferível quando a ordem relativa está disponível, ainda que \texttt{TransactionDT} não revele a data civil real. O IEEE-CIS foi usado como proxy técnico de comércio eletrônico/cartão \cite{ieee_cis_2019}; seus resultados não demonstram generalização para Pix.
...
Para enfrentar essa lacuna, o projeto criou um registro versionado de atributos explicáveis, com fórmula, janela, unidade e limitações das variáveis derivadas. Colunas anônimas permanecem úteis ao classificador, mas não recebem significado de negócio na explicação. A orquestração atual ainda constrói a consulta com os nomes dos fatores de maior influência, inclusive nomes anônimos; essa diferença entre o desenho conceitual e a implementação é tratada como limitação do protótipo.
