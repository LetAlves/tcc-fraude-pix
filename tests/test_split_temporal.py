...
class CorteTemporalTest(unittest.TestCase):
    ...
    def test_cortes_no_mesmo_bloco_reservam_o_proximo_para_validacao(self) -> None:
        quadro = _quadro([1] + [2] * 6 + [3] + [4] + [5])

        treino, val, teste = dividir_temporal(
            quadro, frac_treino=0.3, frac_val=0.3
        )

        self.assertEqual((len(treino), len(val), len(teste)), (7, 1, 2))
        self.assertLess(
            treino["TransactionDT"].max(), val["TransactionDT"].min()
        )
        self.assertLess(
            val["TransactionDT"].max(), teste["TransactionDT"].min()
        )
    ...
    def test_fracoes_invalidas(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "frac_treino"):
            dividir_temporal(_quadro(np.arange(100)), frac_treino=0.0)

        with self.assertRaisesRegex(ValueError, "frac_val"):
            dividir_temporal(_quadro(np.arange(100)), frac_val=-0.1)

    def test_coluna_temporal_ausente(self) -> None:
        with self.assertRaisesRegex(ValueError, "coluna temporal ausente"):
            dividir_temporal(pd.DataFrame({"outra_coluna": [1, 2, 3]}))

    def test_dataset_vazio(self) -> None:
        with self.assertRaisesRegex(ValueError, "dataset vazio"):
            dividir_temporal(_quadro([]))

    def test_timestamp_ausente(self) -> None:
        quadro = _quadro([1.0, np.nan, 3.0])

        with self.assertRaisesRegex(ValueError, "valores ausentes"):
            dividir_temporal(quadro, frac_treino=0.34, frac_val=0.33)
